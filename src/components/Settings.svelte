<script>
	...
	function setShBreak() {
		...
		console.log($shBreak);
		if (typeof window !== 'undefined') {
			localStorage.setItem('shBreak', $shBreak);
		}
	}
	function setLgBreak() {
		...
		console.log($lgBreak);
		if (typeof window !== 'undefined') {
			localStorage.setItem('lgBreak', $lgBreak);
		}
	}
...
<div transition:fade={{ duration: 200 }} class="modal-bg">
	<div class="modal grid grid-row-3 px-12 py-12">
		...
		<div class="lgBreak-settings px-4">
			...
			<div class="minutes-settings-container py-1">
				<input
					bind:value={breakInterval_value}
					class="w-20"
					type="number"
					name="break-minutes"
					placeholder="Minutes"
			...
			<div class="minutes-settings-container pt-1 pb-5 pr-10 grid grid-cols-2 content-start">
				{#if autoStart_value == true}
					<button
						...
						on:click={() => autoStart.set(true)}
						class="w-20 rounded-l-xl bg-blue-600 hover:bg-blue-700 duration-200 text-white font-bold"
						>Yes</button
					>
					<button
						on:click={toggleAutoStart}
						on:click={() => autoStart.set(false)}
						class="w-20 rounded-r-xl bg-gray-300 hover:bg-gray-400 duration-200 text-white font-bold"
						>No</button
					>
				{:else}
					<button
						on:click={toggleAutoStart}
						on:click={() => autoStart.set(true)}
						class="w-20 rounded-l-xl bg-gray-300 hover:bg-gray-400 duration-200 text-white font-bold"
						>Yes</button
					>
					<button
						on:click={toggleAutoStart}
						on:click={() => autoStart.set(false)}
						class="w-20 rounded-r-xl bg-blue-600 hover:bg-blue-700 duration-200 text-white font-bold"
						>No</button
					>
				{/if}
			</div>
		...
		<div
			id="done-button"
			class="justify-around transition ease-in-out delay-150 bg-blue-600 hover:scale-101 hover:bg-blue-700 hover:shadow-xl duration-200"
		>
			<button
				on:click={setPomodoro}
				on:click={setShBreak}
				on:click={setLgBreak}
				on:click={setBreakInterval}
				on:click={toggleSettings}
				class="text-2xl py-4 content-center text-white font-bold"
			>
				Done
			</button>
		</div>
	</div>
	<div on:click={toggleSettings} class="modal-bg-out" />
</div>
